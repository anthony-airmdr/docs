...
* Login into the Okta Admin Consolehttps://your-okta-domain.okta.com

* Enter your admin username and password, then click Sign In. The user must have access to create an Okta API token
  List of RBACs (Role-Based Access Control) to create Okta API token:
  ...
  * Group Member-ship Admins

  * Read Only Admins

* In the Okta Admin Console, in the top navigation bar, click on
...
* Now that you have your API token, you can use it to authenticate API requests. When making API calls, pass the token in the Authorization header as follows:
...
* Go back to the
  ...
  **API Tokens**
  ...
  page in the Okta Admin Console.

* Find the token you want to revoke and click on the

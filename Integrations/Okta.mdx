...
Okta securely connects people to their applications from any device, anywhere, at any time. Okta integrates with existing directories, identity systems, and thousands of on-premises, cloud, and mobile applications. It runs on a secure, reliable, and extensively audited cloud-based platform.

## Create Okta API Token

* Login into the Okta Admin Console -->[https://your-okta-domain.okta.com](https://your-okta-domain.okta.com)

* Enter your admin username and password, then click **Sign In**.

> The user must have access to create an Okta API token. The list of RBACs (Role-Based Access Control) to create Okta API tokens are as follows:
